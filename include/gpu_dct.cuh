...
public:
  StreamMemoryPool() : m_pool(nullptr) {
    ...
    throw_on_cuda_error(cudaMemPoolSetAttribute(m_pool,
...
public:
  ...
  void batch_dct_host(const T *h_images, uint64_t *h_hashes, int batch_size,
    ...
    batch_dct_host(
        std::span<const T>{h_images,
                           static_cast<size_t>(m_size * m_size * batch_size)},
        std::span<uint64_t>{h_hashes, static_cast<size_t>(batch_size)}, stream);
  }
