...
__global__ void kernel_generate_dct_transform(T* output, int n) {
    ...
    const T c_i = (i == 0) ? sqrt(static_cast<T>(1) / n) : sqrt(static_cast<T>(2) / n);
 ...
 */
template<typename T>
__global__ void kernel_fused_dct_64x64(
    const T* __restrict__ input,
    T* __restrict__ output,
    const T* __restrict__ transform,
    int batch_size,
...
__global__ void kernel_fused_dct_general(
    const T* __restrict__ input,
    T* __restrict__ output,
    const T* __restrict__ transform,
    int batch_size,
