...
void benchmark_multistream(int n, int batch_size, int num_streams) {
  ...
  cudaMemcpy(d_images, h_images.data(), n * n * batch_size * sizeof(T),
...
void benchmark_comparison() {
  ...
  for (int n : sizes) {
    ...
    std::cout << std::left << std::setw(20)
              ...
              << time_per_image_us << std::setw(18) << std::fixed
              << std::setprecision(1) << throughput_k << std::endl;
  }
}
...
void print_device_info() {
  ...
  if (memory_clock_khz > 0) {
    std::cout << "Memory Clock Rate: " << (memory_clock_khz / 1000) << " MHz"
              << std::endl;
  } else {
  ...
  if (cudaDeviceGetAttribute(&memory_bus_width, cudaDevAttrGlobalMemoryBusWidth,
                             device) != cudaSuccess) {
    memory_bus_width = 0;
  }
